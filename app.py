...
def main():
    ...
    classifier = st.sidebar.selectbox("Classifier", ("Support Vector Machine(SVM)","Logistic Regression", "Random Forests" ))
    
    if classifier == "Support Vector Machine(SVM)":
        st.sidebar.subheader("Model Hyperparameters")
        C = st.sidebar.number_input("C (regularization parameter)", 0.01, 10.00, step = 0.01, key = "C")
    ...
    if st.sidebar.checkbox("Make your own prediction", False):
        ...
        age = st.number_input("age:", value=0, min_value=0, max_value=100, step = 1, key = 'age')
        sex = st.number_input("sex:", value=0, min_value=0, max_value=1, step = 1, key = 'sex')
        cp = st.number_input("cp:", value=0, min_value=0, max_value=3, step = 1, key = 'cp')
        trest_bps = st.number_input("trest_bps:", value=100, min_value=100, max_value=200, step = 1, key = 'trest_bps')
        chol = st.number_input("chol:", value=100, min_value=100, max_value=450, step = 1, key = 'chol')
        fbs = st.number_input("fbs:", value=0, min_value=0, max_value=1, step = 1, key = 'fbs')
        rest_ecg = st.number_input("rest_ecg:", value=0, min_value=0, max_value=1, step = 1, key = 'rest_ecg')
        thalach = st.number_input("thalach:", value=80, min_value=80, max_value=210, step = 1, key = 'thalach')
        exang = st.number_input("exang:", value=0, min_value=0, max_value=1, step = 1, key = 'exang')
        old_peak = st.number_input("old_peak:", value=0, min_value=0, max_value=7, step = 1, key = 'old_peak')
        ...
        thal = st.number_input("thal:", value=0, min_value=0, max_value=3, step = 1, key = 'thal')
